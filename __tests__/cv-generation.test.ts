...
const mockDocument = jest.fn(({ children }) => <div data-testid="pdf-document">{children}</div>)
const mockPage = jest.fn(({ children, style }) => <div data-testid="pdf-page" style={style}>{children}</div>)
const mockText = jest.fn(({ children, style }) => <span data-testid="pdf-text" style={style}>{children}</span>)
const mockView = jest.fn(({ children, style }) => <div data-testid="pdf-view" style={style}>{children}</div>)
const mockStyleSheet = { create: jest.fn((styles) => styles) }
    ...
    mockFrom.mockImplementation((table: string) => {
      ...
      if (table === "track_records") {
        return {
          select: jest.fn().mockReturnValue({
            eq: jest.fn().mockReturnValue({
              order: jest.fn().mockResolvedValue({ data: trackRecords, error: null }),
            }),
          }),
        }
      }
      if (table === "tracer_study_responses") {
        return {
          select: jest.fn().mockReturnValue({
            eq: jest.fn().mockReturnValue({
              maybeSingle: jest.fn().mockResolvedValue({ data: tracerStudy, error: null }),
            }),
          }),
        }
      }
...
describe("TC 5.2 — CvTemplate Component Rendering (ID)", () => {
  ...
  it("should render document with profile name as title", () => {
    const { container } = renderWithWrapper(<CvTemplate data={mockData} lang="id" />)
...
describe("TC 5.2b — CvTemplate Component Rendering (EN)", () => {
  ...
  it("should render English labels when lang='en'", () => {
    ...
    for (const label of enLabels) {
      const calls = mockText.mock.calls.filter(
        (args: any[]) => args[0]?.children === label
      )
...
describe("TC 5.3 — CvTemplate Missing Optional Fields", () => {
  ...
  it("should not render NIM when profile.nim is null", () => {
    ...
    const nimCalls = mockText.mock.calls.filter(
      (args: any[]) => typeof args[0]?.children === "string" && args[0]?.children?.includes("12345")
    )
